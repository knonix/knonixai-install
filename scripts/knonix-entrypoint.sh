...
// the answer renderer when status is ready (live UI).
{
  ...
  // 4b) Do NOT inject related follow-ups in the client answer renderer.
  // Display-time inject has no user query, so it ground buttons on answer prose
  // ("Explain more about To assess your CMMC…") and often dumps raw ```spec.
  // Server save path (4a) has userQuery — keep that only.
  //
  // Also strip any *already-applied* bad UI inject (from older entrypoint runs
  // on a writable layer) by restoring plain MarkdownMessage props.
  const undoUi = [
    {
      label: "related-followups-ui-static-undo",
      // Match injected form (with ensure fn) → plain message:e
      // We only undo the known wrapper pattern if present; safe no-op otherwise.
      old: null,
      new: null,
    },
  ];
  ...
  // Undo previous display-time inject: replace ensure-wrapper with plain props.
  // Patterns are long; match a distinctive marker then rewrite via regex.
  for (const f of allFiles) {
    ...
    if (!data.includes("MarkdownMessage") || !data.includes("submitQuery")) {
      continue;
    }
    // Static: message:("streaming"===c||"submitted"===c)?e:(function(...)(e)
    // SSR: message:("streaming"===j||"submitted"===j)?a:(function(...)(a)
    const reStatic =
      /\(0,t\.jsx\)\(k\.MarkdownMessage,\{message:\("streaming"===c\|\|"submitted"===c\)\?e:\(function\(t,u\)\{[\s\S]*?\}\)\(e\),citationMaps:u\}\)/;
    const reSsr =
      /\(0,b\.jsx\)\(x\.MarkdownMessage,\{message:\("streaming"===j\|\|"submitted"===j\)\?a:\(function\(t,u\)\{[\s\S]*?\}\)\(a\),citationMaps:l\}\)/;
    let next = data;
    if (reStatic.test(next)) {
      next = next.replace(
        reStatic,
        "(0,t.jsx)(k.MarkdownMessage,{message:e,citationMaps:u})"
      );
      console.log(
        `[knonix-entrypoint] related-followups-ui-static: undone ${path.basename(f)}`
      );
    }
    if (reSsr.test(next)) {
      next = next.replace(
        reSsr,
        "(0,b.jsx)(x.MarkdownMessage,{message:a,citationMaps:l})"
      );
      console.log(
        `[knonix-entrypoint] related-followups-ui-ssr: undone ${path.basename(f)}`
      );
    }
    if (next !== data) fs.writeFileSync(f, next);
  }

  if (!saveOk) {
    console.log("[knonix-entrypoint] related-followups: save pattern not found");
  } else {
    console.log("[knonix-entrypoint] related-followups: save=true ui=disabled");
  }
}

// --- 4c) Chat waiting status: ONE "Working" label (no Connecting/Thinking rotation) ---
// Stock UI rotates labels + shows multiple bounce-dot groups. Keep a single verb.
{
  const OLD_WAIT =
  ...
  // Also collapse prior "simple" patch that still flipped detail after 20s
  const OLD_WAIT2 =
  ...
  for (const f of allFiles) {
    ...
    if (patchFile(f, OLD_WAIT2, NEW_WAIT, "waiting-status-simple")) waitOk = true;
  }
  if (!waitOk) console.log("[knonix-entrypoint] waiting-status-simple: pattern not found");
}

// --- 4c2) Collapse ChatWaitingStatus to ONE dots group + Working + progress ---
// Stock minified UI: header dots + body md dots + phase-label sm dots (triple).
// Keep header (dots + Working + elapsed) and a slim progress bar only.
{
  const OLD_BODY =
    '(0,i.jsxs)("div",{className:"flex items-start gap-3 text-sm",children:[(0,i.jsx)(j,{size:"md",className:"mt-1.5 shrink-0"}),(0,i.jsxs)("div",{className:"min-w-0 flex-1",children:[(0,i.jsxs)("div",{className:"flex items-center gap-2 font-medium text-foreground",children:[(0,i.jsx)("span",{className:"animate-in fade-in duration-200",children:u.label}),(0,i.jsx)(j,{size:"sm",className:"text-muted-foreground"})]}),(0,i.jsxs)("div",{className:"mt-0.5 text-xs text-muted-foreground",children:[u.detail,c>=10?" · First load of a local model is often the slowest step.":""]}),(0,i.jsx)("div",{className:"mt-2 h-1.5 w-full overflow-hidden rounded-full bg-muted",children:(0,i.jsx)("div",{className:"h-full rounded-full bg-primary/60 transition-[width] duration-500 ease-out",style:{width:`${m}%`}})})]})]})';
  // Progress-only body (no second/third bounce groups, no dual Working label)
  const NEW_BODY =
    '(0,i.jsx)("div",{className:"mt-2 h-1 w-full overflow-hidden rounded-full bg-muted",children:(0,i.jsx)("div",{className:"h-full rounded-full bg-primary/60 transition-[width] duration-500 ease-out",style:{width:`${m}%`}})})';
  ...
  for (const f of staticFiles) {
    ...
    let next = data;
    next = next.split('label:"Connecting"').join('label:"Working"');
    next = next.split('label:"Thinking"').join('label:"Working"');
    next = next.split('label:"Still working"').join('label:"Working"');
    if (next.includes(OLD_BODY)) {
      next = next.split(OLD_BODY).join(NEW_BODY);
      console.log(
        `[knonix-entrypoint] waiting-single-row: patched ${path.basename(f)}`
      );
      tripleOk = true;
    }
    if (next !== data) {
      ...
      if (!tripleOk) {
        console.log(
          `[knonix-entrypoint] waiting-labels-collapse: patched ${path.basename(f)}`
        );
      }
      tripleOk = true;
    }
  ...
  if (!tripleOk) {
    console.log("[knonix-entrypoint] waiting-single-row: pattern not found");
  }
}
